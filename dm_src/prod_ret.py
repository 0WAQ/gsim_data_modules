...
class DmgrProdret(DataManagerMapped):
    def __init__(self, ):
        DataManagerMapped.__init__(self, )
        self.days = None
        self.me = None
        self.prodret = NIO_MATRIX()
        return
    ...
    def initialize(self, id, path, cfg):
        ...
        self.addDailyData(self.prodret, self.tag)
        return
    ...
    def loadData(self, di_start):
        ...
        for di in range(di_start, len(uv.Dates)):
            ...
            if di < self.days:
                continue
            self.prodret[di] = Oputil.prod(ret[di - self.days + 1:di + 1], self.me)
            print ("ret_di:",ret[di])
            print (di, self.prodret[di])
        return

...
class DmgrStdret(DataManagerMapped):
    def __init__(self, ):
        DataManagerMapped.__init__(self, )
        self.days = None
        self.me = None
        self.stdret = NIO_MATRIX()
        return
    ...
    def initialize(self, id, path, cfg):
        ...
        self.addDailyData(self.stdret, self.tag)
        return
    ...
    def loadData(self, di_start):
        ...
        for di in range(di_start, len(uv.Dates)):
            ...
            if di < self.days:
                continue
            self.stdret[di] = Oputil.std(ret[di - self.days + 1:di + 1], self.me)
        return

...
class DmgrInterval5m(DataManagerMapped):
    def __init__(self, ):
    ...
    def initialize(self, id, path, cfg):
        ...
        self.addData(self.ret, self.tag + '.ret', len(uv.Dates), len(uv.Instruments), uv.INTRADAY_STEPS_5M)
        self.addData(self.pctchange, self.tag + '.pctchange', len(uv.Dates), len(uv.Instruments), uv.INTRADAY_STEPS_5M)
        self.addData(self.vwap, self.tag + '.vwap', len(uv.Dates), len(uv.Instruments), uv.INTRADAY_STEPS_5M)
    ...
    def registration(self, ):
        ...
        dr.registerData(self.mid, self.ret, self.tag + '.ret')
        dr.registerData(self.mid, self.pctchange, self.tag + '.pctchange')
        dr.registerData(self.mid, self.vwap, self.tag + '.vwap')
        return
    ...
    def loadDay(self, di):
        ...
        for line in infile:
            ...
            code = raw_code[0:6]#+'.'+raw_code[0:2]
            ...
            timeStr = linespt[header.index('datetime')][11:16]
            #print ("timeStr:",timeStr)
            #print ("timeIdx:",timeIdx)
            if timeStr not in timeIdx:
            #    print (timeStr not in "timeIdx")
                continue
            ti = timeIdx[timeStr]
            #print ("ti=",ti)
            ...
            amoStr = linespt[header.index('turnover')]
